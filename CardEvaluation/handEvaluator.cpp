...
string evaluateHand(const vector<int>& hand) {
    ...
    // we don't know the size of ranks. is is because we exclude duplicate ranks, because they could break this check
...
int handWinner(const std::vector<std::vector<int>>& playerHands) {
    ...
    for (size_t i = 1; i < playerHands.size(); ++i) {
        ...
        if (currentRank > bestRank) {
            ...
            // we need some special check for ace-low flushes/straight flushes. These should score aces differently than in all other cases.
